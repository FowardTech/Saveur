...
const LearningCourses = memo(() => {
  ...
  // PRODUCT FEEDBACK: "the AI curriculum builder should only show when a
  // goal and a role is set because it can confuse the user with the learn
  // anything feature" — previously auto-built from EITHER signal alone
  // (see the OR below, still used for the actual generation text once
  // both are confirmed present) and fell back to a manual "type your
  // goal" box whenever NEITHER was set, which duplicated "Learn anything"
  // right below it with nothing to tell the two apart at a glance. Now
  // requires BOTH a stated goal AND a desired role before the whole card
  // appears at all — see showCurriculumBuilder below, which still keeps
  // showing it once a curriculum has actually been generated, so this
  // only gates the fresh-start entry point, never already-earned
  // progress.
  ...
  React.useEffect(() => {
    if (!curriculumLoaded || curriculum || hasAutoGeneratedRef.current || !hasGoalAndRole) return;
  ...
  return (
    <Container style={styles.container}>
      ...
      <Content padder avoidKeyboard contentContainerStyle={styles.content}>
        ...
        {curriculumLoaded && (hasGoalAndRole || !!curriculum) ? (
